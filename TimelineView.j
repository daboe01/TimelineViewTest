...
@implementation TLVTimeLane : CPView
{
    ...
    CPRange      _fixedValueRange @accessors(property=fixedValueRange);
...
- (void)_drawVerticalRuler
{
    ...
    if (!_valueRange || CPMaxRange(_valueRange) >= HUGE_NUMBER || _valueRange.location <= HUGE_NUMBER*(-1) || _valueRange.location >= HUGE_NUMBER)
        return;
 
    var effectiveValueRange = _fixedValueRange? _fixedValueRange : _valueRange;

    var tickCount = MAX(5, Math.ceil(pixelHeight / 30));
    var tickSize = effectiveValueRange.length / (tickCount - 1);
    var roundedTickRange = [self _roundValue:tickSize];

    var gapBetween = pixelHeight * (roundedTickRange / effectiveValueRange.length);
    var yLabel = [self _roundValue:CPMaxRange(effectiveValueRange)];
    ...
    for (var y = pixelHeight * ((CPMaxRange(effectiveValueRange) - yLabel) / effectiveValueRange.length); y < pixelHeight; y += gapBetween, yLabel -= roundedTickRange)
    {
        if (y < 1)
            continue;
...
// fixme: clip by getDateRange
    ...
    if( lane._fixedValueRange)
    {   minY = lane._fixedValueRange.location;
        maxY = CPMaxRange(lane._fixedValueRange);
    }
